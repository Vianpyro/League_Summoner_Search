async function displayChampions() {
    try {
        ...
        document.getchampionById("champ-toptext").innerHTML = ("With " + champions.length + " champions, you’ll find the perfect match for your playstyle. Master one, or master them all.")
        ...
        champions.slice().forEach((champion, i) => {
            ...
            document.getchampionById(`champions`).innerHTML +=
                `<figure>
                    ...
                    onerror="javascript: document.getchampionById('get${champion.name}Icon').src = ${on_error_link}">
                    <figcaption id="champ${champion.name}Name"></figcaption></a>
                </figure>`;

            if (champion.displayName) {
                document.getchampionById(`champ${champion.name}Name`).innerHTML = `${champion.displayName}`;
            } else {
                document.getchampionById(`champ${champion.name}Name`).innerHTML = `${champion.name}`;
            }
        });
    } catch (err) {
        console.error(err);
